...
** Customize desktop files
  #+begin_src sh :results output silent
    ...
    cat /usr/share/applications/google-chrome.desktop  \
        | sed 's|Exec=/usr/bin/google-chrome-stable|Exec=/usr/bin/google-chrome-stable --enable-features=UseOzonePlatform --ozone-platform=wayland --password-store=gnome|' \
