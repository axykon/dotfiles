...
* Install kustomize
  #+begin_src sh :var version="v3.8.2" :results output silent :dir /sudo::/tmp
    set -e
...
* Install scaffold
  #+begin_src sh :results output silent :dir /tmp
    set -e
...
* Install helm
  #+begin_src sh :var version="v3.3.1" :results output silent :dir /sudo::
    set -e
...
* Install istio
  ...
  #+end_src
  Then run ~istio install --set profile=demo --set values.global.imagePullPolicy=IfNotPresent~
